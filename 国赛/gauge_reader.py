...
def _status_from_color_bands(roi: Any, pointer_angle: float, cx: float, cy: float, radius: float) -> str | None:
    ...
    red1 = cv2.inRange(hsv, np.array([0, 80, 45]), np.array([12, 255, 255])) > 0
    red2 = cv2.inRange(hsv, np.array([168, 80, 45]), np.array([179, 255, 255])) > 0
    red_mask = (red1 | red2) & annulus
    yellow_mask = (cv2.inRange(hsv, np.array([12, 25, 35]), np.array([55, 255, 255])) > 0) & annulus
